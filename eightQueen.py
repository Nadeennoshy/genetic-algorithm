...
def init_pop(pop_size):         #pop_size = number of rows
    return np.random.randint(8, size =(pop_size,8))    #random numbers between 0 and 7
initial_population = init_pop(4)
#print(initial_population)

## Calculate fitness function
def calc_fitness(population):
    fitness_vals = []              # store values of fitness function in this llist
    for x in population:
        penalty = 0
        for i in range(8):
            r = x[i]
...
def selection(population,fitness_vals):
    ...
    N = len(population)
    indices = np.arange(N)                # if N=4 indices=0,1,2,3
...
def crossover(parent1,parent2,pc):         #pc:probability of crossover
    r = np.random.random()                 # return random numbers between 0 and 1
    if r < pc:
        m = np.random.randint(1,8)         # return random numbers between 1 and 8
...
#testing our function with 2 parents from selected_population
...
## mutation
def mutation(individual,pm):           #pm:probability of mutation
    r = np.random.random()
    if r < pm:
        m = np.random.randint(8)        # random numbers between 0 and 7
        individual[m] = np.random.randint(8)
    return individual

## apply croosover and mutation
def crossover_mutation(selected_pop,pc,pm):
    N = len(selected_pop)
    new_pop = np.empty((N,8),dtype=int)         # new_pop has the same size of population
    for i in range(0,N,2):                     # N: number of individuals
